...
### Step 0.5：讀取報告文件（.docx 或 .md，若以檔案路徑提供）
...
- 資料 2：instruction 中 `Upstream_Required` 列出的所有上游報告（上傳 `.docx` 或貼上內容）
...
- 資料 2：既有研究報告（上傳 `.docx` 或貼上報告內容，讓 Gemini 知道哪些已有結論，避免重複研究）
...
2. 既有研究報告（上傳 .docx 或貼上報告內容，作為脈絡基準）
