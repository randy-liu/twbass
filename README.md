...
上述研究報告（`*_*.md`）是給 Gemini／稽核用的「素材」；另外有一條**讀者向**的產出線，把這些素材重新綜合成不同程度釣手能讀的白皮書，並產生一個自含的網站。

| 檔案 | 受眾 | 定位 |
|------|------|------|
| `台灣大嘴黑鱸白皮書-junior.md` | 路亞初階釣手 | 實戰決策、白話 how-to |
| `台灣大嘴黑鱸白皮書-senior.md` | 路亞中階／資深釣手 | 精確閾值與機制「為什麼」 |
| `台灣大嘴黑鱸白皮書-expert.md` | 專業／競技釣手 | 逆向診斷、信心校準、決策科學 |

- 科學詞彙一律以**國中程度**白話解釋，章節開頭先宣告一次；senior／expert 另以「💡 深入一點（可跳過）」側欄提供進階註解，主文不加深。
- 重建網站：`python build.py` → 產生 `docs/index.html`（三階分頁、亮暗雙主題、術語速查與自動連結、襯線標題；單一自含檔，除 Google Fonts 無外部資源）。
- 三份共通的數值（如溶氧 mg/L、底質 Eh／爛底毒區）皆附「不用儀器的現場判讀法」（溶氧看時間/水溫/風流/水色/浮頭；爛底靠搆得到的泥聞臭、自然冒泡、岸邊水面，深水則以季節＋滯水推定），避免讀者拿到無法落地的數字。
...
> 🔄 **v3 全庫重跑（2026-07-03 起，換模型）：** 所有可執行 instruction（0A–4B、SUP-A–SUP-E）已完成重跑前內容可靠性稽核（見 `PATCH_NOTES.md` 2026-07-02 條目），現全數標為 **v3 待重跑**。下表「現行版本」欄為 v2 產出，在對應卷的 v3 報告產出並通過稽核前，維持為現行版本與下游上游輸入；v3 報告完成後逐一取代。
