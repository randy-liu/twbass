...
- **軟殼期暴利**：沼蝦/螯蝦脫殼的軟殼期，處理時間從 1.2–2.0 s 暴跌到 0.2 s，CP 值躍升 5–13 倍。水溫 >26°C 觸發脫殼潮，魚會專一搜尋軟殼個體——這是夏季結構區用帶鹽超軟蝦/蟹軟蟲的學理。
...
<a id="senior-cff"></a>
**速度有上限，由魚眼的「更新率」CFF 決定。** 前面講越快越好，但快是有天花板的。魚眼跟相機一樣有畫面更新率（CFF，臨界閃爍融合頻率，Hz）——每秒能分辨多少次變化。關鍵是 CFF 不是固定值，會隨水溫、光線大幅升降：**高溫晴天約 62–68 Hz（取樣快、看得清細節），低溫 15°C 或黃昏弱光掉到 35–42 Hz（取樣慢、只剩黑白輪廓）**。同一個餌速，落在不同 CFF 上、結果完全相反，而且兩端各有一條陷阱：

- **CFF 低（冷水、黃昏、深暗水）——快餌會「糊掉」**：取樣太慢，快速移動的餌在一幀內位移太多，糊成一條拖影（像慢快門把車燈拉成線），魚追不上、也讀不出「那是隻在逃的獵物」。所以**冬天和黃昏要把抽餌速度降一檔**——不是放慢去討好慢通路，而是慢到落進魚眼當下解析得出來的更新率內，牠才追得到、咬得下。夏季正午那套「越快越炸」的節奏，搬到冷水或弱光只會變成一連串空槍。
- **CFF 高（高溫、亮場、清水）——快餌會「被看穿」**：取樣快，餌被看得又清晰、又像慢動作逐格呈現，等於**給了慢通路充裕的清晰畫面去審查**——認出這是假的、想起被釣痛的記憶，於是「只跟不咬」。所以清水亮場光靠絕對速度沒用，得**靠抖停加速製造突發逃逸訊號**（≥2.5 m/s²），趕在慢通路看穿之前先點燃快通路。

> ⚠️ 容易被誤導的一點：高 CFF 看起來「像慢動作」，**不代表魚會把在動的餌看成靜止、判定不能吃而放棄**。慢動作 ≠ 不動——餌在魚眼裡照樣換位置。高 CFF 真正讓魚放掉假餌的原因是「**看得太清楚被識破**」，不是「看成不動」；這也是為什麼 free fall 慢沉、dead-stick 死等這種幾乎不動的呈現照樣中魚（見 §十一）。

判斷不用儀器，用水溫＋天色推 CFF 高低：**水溫低於約 15°C、或清晨日出前／黃昏日落後那段弱光 → CFF 低 → 放慢、把動作做大**；**高溫晴天的清水亮場 → CFF 高 → 別只拚速度，改用抖停加速的「變化」去逼反射。**

> 這裡講的是 CFF 對**速度決策**的影響；同一個 CFF 還決定**餌的色彩與寫實度**該怎麼選（剪影 vs 寫實），那部分見 [〈§二十 視覺端的修正：CFF 與水色〉↓](#senior-cff-color)。
...
| 老魚（反覆釣放） | 20–40 ng/mL（台灣高壓計費池實測集中 35–45） | 75–90% | 15–30% |

皮質醇基線其實分**三個各自有出處的層級**，別混為一談：健康未受壓個體 **1.68±0.69 ng/mL**、剛放流 <30 天仍帶放流緊迫的新魚 **10–15 ng/mL**、反覆釣放重塑後的老魚 **20–40 ng/mL**（台灣高壓計費池實測集中 35–45）。兩個行為欄位的意思：**Follower Rejection** = 跟到嘴邊卻臨陣拒咬的機率；**不匹配餌反射咬率** = 對「長得不像當下天然獵物」的假餌仍反射性出擊的機率（越高＝越好騙，所以新魚什麼都咬、老魚非寫實不咬）。
...
**連投節奏——同一個點要隔多久再打。** 魚被釣一次後，主動覓食的驅力要過一段時間才恢復，這段時間叫 ART（Alert Reset Time，警覺重置時間）。ART 由皮質醇從血裡被清除的速度決定，而清除速度隨水溫變（Q₁₀=2.0，水溫每升 10°C 約快一倍）：重度釣獲後 5°C 要 67.9 hr、15°C 36–42 hr、22°C 20–24 hr、30°C 12 hr、35°C 8.5 hr。由此導出**同一標點的安全連投間隔**：北部冬季管理池（15°C）≥10–20 min、南部夏季（30°C）≥3–5 min。打得比這更密，皮質醇會一階一階往上疊（前一次還沒清完又再加），魚 100% 進入 Lure-shyness（見餌就走）。

**警報素死區——中魚/跑魚會在水裡留下化學警報。** 魚受傷或激烈掙扎會釋放警報素（Schreckstoff），讓附近同類集體閉口、避開那塊水，形成一個「死區」（魚不敢進的範圍）。死區大小與恢復時間南北差很多——北部酸水＋低 CDOM 會把警報素快速鈍化、光解掉，南部中性水＋高 CDOM 反而幫它保鮮：

- **（台北/基隆/宜蘭）**：死區半徑 3.35 m、約 10–15 min 恢復。
- **（桃竹苗）**：死區半徑 3.90 m、約 10–15 min 恢復。
- **（高雄/台南/屏東）**：死區半徑 4.91 m、30–45 min 才恢復、警報素活性比例高達 94%。
...
> **第四部精要：** 老魚難釣＝慢性高皮質醇把端腦鎖在風險否決。繞過大腦（脫習慣化 ΔV≥35 cm/s、加速度 ≥2.5 m/s²、結構 <12 cm 壓縮決策）或徹底擬真（Shore 00-15 超軟餌、尺寸優先、1–5 Hz 微振動、Dead Drift）。守住 ART 連投間隔（南 3–5、北 15–20 min）與警報素換點（南 ≥8 m）。記住皮質醇閘門：剛被擾過的水死氣沉沉、久封＋飢餓的水瘋狂。
...
黑鱸的繁殖在台灣先天就吃虧，再被後天環境夾殺，這就是「雙重打擊」。

**第一重：先天缺「夠用面積」的硬巢底——但不是缺到沒有。** 黑鱸築巢只要是**硬底**都能用：天然砂礫最理想，但岸際植物根系、沉木，甚至人工的磚塊、拋石、水泥護岸都照用不誤（這正是「人工產房墊」會有效的原因）。問題出在**面積比例**：台灣水體不管哪種硬底加起來，通常只佔池底一小塊——野生埤塘約 2.0–4.5 m²/每 100 m²、深水水庫 0.5–2.0、管理池 1.0–3.0（多為人工磚塊/水泥斜坡），北美原棲地則常 >40 m²/每 100 m²，等於台灣 >90% 的池底是軟泥。所以這是「**全池能用的硬底太少、攤到整個族群不夠分**」的面積問題，**不是土壤化學問題、也不是沒有硬底可用**。

- 真的被擠到軟泥上做巢，孵化率只剩 12–28%（軟泥窒息、易被泥覆蓋）；用礫石硬底有 75–90%；用植物根系這類**替代硬基底**居中，約 35–55%。
- 換句話說，一口結構豐富（混凝土岸、拋石、廢棄硬物、發達根系）的池塘，黑鱸找得到好巢位，繁殖就正常——這跟人工產房墊的邏輯一致。

**第二重：後天的南北環境夾殺——但都是「事件型」，不是天天發生。**

- **南部——春雨回淹後的 H₂S 撞上產卵期。** 南部埤塘旱季乾掉、春雨回淹後 10–20 天，底泥還原把游離 H₂S 推到 0.15–0.85 mg/L 的峰值（約 3 月中–4 月初），這個毒性高峰和產卵高峰重疊 >80%，軟泥巢失敗率 60–80%。
- **北部——春雨「濁水＋酸徑流」的雙殺，但要剛好撞上才成立：**
    - **黏土裹卵（要濁水才會發生）。** 春雨沖刷把大量高嶺石黏粒帶進池子（懸浮泥 SS＝水中懸浮固體濃度達 100–300 mg/L 時），黏粒 12 hr 內在卵表結成 80–180 μm 的裹層、氧氣擴散掉 72.5%，孵化率折損 65–85%。**但這完全看懸浮泥濃度**：SS <50 mg/L（也就是水清的時候）孵化率仍有 80–90%——所以**常年清澈、排水良好的池塘基本躲掉這一刀**。
    - **酸化壓低受精率（看水酸到什麼程度，而且數據是外推）。** 春雨酸徑流若把水體短暫壓到 pH 6 以下，會抑制精子活力、拉低受精率。但有兩點要留意：一是這組「pH→受精率」主要是**從近緣魚種推估**的，黑鱸本身其實相當耐酸（北美大量 pH 5.5–6.5 的茶色酸水照樣繁殖旺盛）；二是**土酸不等於水酸**——北部常態池水多在 pH 5.5–6.5，清水、藻相活躍的池塘更接近中性，未必跌進崩盤區。所以這是「春雨強酸事件」才觸發的折損，不是北部池塘的常態判決。

**兩個南北共通的致命傷：** 護巢公魚對水位極敏感，水位日降 ≥15 cm/day（水庫洩洪、管理池放水）棄巢率高達 80–95%；倒春寒水溫一跌破 13.5°C，母魚立刻停止排卵。

**所以實務上：** 一口排水良好、長年清澈、緩衝穩定、又沒在產卵期碰上暴雨脈衝的池塘，就算長在酸性紅土上，黑鱸一樣能正常繁殖、族群穩定。
...
> **第五部精要：** 先天硬巢底面積不足（被擠到軟泥時孵化率才掉到 12–28%；有結構/人工硬底就正常）＋後天的春雨事件型夾殺（南部 H₂S 撞產卵期、北部濁水裹卵與強酸徑流，但都要剛好撞上才成立）＋外來種壓縮＋吃卵＋無禁釣期打巢，疊起來造成補充損耗。資深釣手的價值不只在技術，也在於知道什麼時候該收手。
...
CFF（魚眼更新率）的定義與它對**速度決策**的影響，已在 [〈§九 速度有上限〉↑](#senior-cff) 講過（高溫 62–68 Hz 看得清快速細節、低溫 15°C 或黃昏降到 35–42 Hz 退化成黑白高對比輪廓）。這裡接著講同一個 CFF 的**另一面向——餌的色彩與寫實度該怎麼選**，並配合水色：
