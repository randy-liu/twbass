...
<a id="senior-cff"></a>
**速度有上限，由魚眼的「更新率」CFF 決定。** 前面講越快越好，但快是有天花板的。魚眼跟相機一樣有畫面更新率（CFF，臨界閃爍融合頻率，Hz）——每秒能分辨多少次變化。關鍵是 CFF 不是固定值，會隨水溫、光線大幅升降：**白天暖水（約 30°C）最高、可達 62–68 Hz（取樣快、看得清細節）；冷水 15°C 降到 35–42、極端高溫（>35°C）因熱緊迫又反降；而一旦入夜或黃昏弱光，不論水溫都直接暴跌到 5–15 Hz（取樣慢、只剩黑白輪廓）**。同一個餌速，落在不同 CFF 上、結果完全相反，而且兩端各有一條陷阱：

- **CFF 低（冷水、黃昏、深暗水）——快餌「糊掉」，根本輪不到咬。** 取樣太慢，移動稍快的餌一幀內就位移太多、糊成一條拖影（像慢快門把車燈拉成線）；餌自帶的抖動也會超過 CFF、被抹平成不抖的死樣子。魚追不上、也讀不出「在逃的獵物」，自然不出手。所以**冬天和黃昏要把速度降一檔、改用低頻軟餌**，讓動作落回魚眼當下解析得出來的範圍，牠才追得到、咬得下。
- **CFF 高（暖水約 30°C、亮場、清水）——魚把餌看得太清楚，光靠速度騙不過。** 先講一個前提：餌要被「看清、甚至像慢動作逐格呈現」，牠的移動速度與抖動頻率必須**遠低於**魚當下的 CFF——只是勉強壓在門檻下還不夠，越貼近 CFF 就越開始發糊；差距要拉得夠大，魚才解析得出每一個細節。而暖水亮場的清水裡，CFF 本來就高、水又讓魚看得遠看得久，**一般的收餌速度對牠來說都還在「看得清」的範圍，你再怎麼加快，也快不到讓牠看糊的程度**。結果就是：魚得到一個又清晰又持久的畫面，慢通路（大腦審查）有充足的時間與畫質把餌審完——認出是假的、想起被釣痛的記憶，於是**只跟不咬**。
    所以清水亮場**純拚絕對速度沒用**（那只是把一個被看得一清二楚的目標，送去給牠慢慢審查而已）。唯一的破法是**抖停加速**：製造一個突發的逃逸訊號（加速度 ≥2.5 m/s²），搶在慢通路審查完成之前，先點燃不經大腦的反射快通路。

**所以「夏季正午越快越炸」要這樣理解：** 正午水暖、CFF 拉到最高，魚能解析的速度天花板也最高，你才敢放膽快抽、用速度逼反射。但這個「快」**有天花板（就是 CFF）、不是無限**——一旦快到超過 CFF，餌就糊成拖影、魚根本讀不出來、也就不咬；水一冷、光一弱，天花板一低，同樣的快立刻變成一連串空槍。

**而且要認清你實際能控制的只有三件事：** 收線**速度**、給餌一個**加速度/急停**（抖停）、和**變向**。餌自帶的抖動頻率（crank 的擺、軟餌的尾震）是**選餌**時就定了的，收線途中沒法精準調。所以 CFF 這一課的實戰結論不是「去調頻率」，而是——**按水溫天色把速度壓在 CFF 天花板以內、暗場冷水改用低頻餌，再靠抖停加速與變向去當扳機。**

**舉個具體的：rubber jig（橡膠裙擺 jig）抖竿尾。** 你的手再怎麼快也只抖得出 2–5 Hz 的大節奏（裙擺一張一縮），真正高頻的細抖是**裙擺那幾十根矽膠絲自己顫動出來的**——高頻來自餌、不是你的手（這也是為什麼「抖竿尾控 CFF」是錯的：CFF 是魚的眼、你動不了，你動的只是餌的訊號）。而魚分不分得出這些細抖，由牠當下的 CFF 決定：暖水亮場 CFF 高，裙擺的細顫牠看得一清二楚、像活蟹在動；冷水或黃昏 CFF 一低，連那些細顫都糊成一團——**這時的解法不是把竿抖更快（你也抖不快、抖快了照樣糊），而是放慢節奏、多停多沉，或換慢擺的裙。**
...
判斷不用儀器，用水溫＋天色推 CFF 高低：**水溫偏冷（<15°C）、清晨/黃昏日落後那段弱光、或盛夏極端高溫（>35°C）→ CFF 低 → 放慢、把動作做大**；**暖水（約 25–30°C）晴天清水亮場 → CFF 最高 → 別只拚速度，改用抖停加速的「變化」去逼反射。** 注意：你是溫血、魚是冷血——天冷時你視力照常，魚的 CFF 卻已被冷水壓低，別用自己的眼睛去推牠看不看得清。
...
CFF（魚眼更新率）的定義與它對**速度決策**的影響，已在 [〈§九 速度有上限〉↑](#senior-cff) 講過（白天暖水約 30°C 達 62–68 Hz、看得清快速細節；冷水 15°C 降到 35–42、入夜或黃昏弱光更暴跌到 5–15 Hz、退化成黑白高對比輪廓）。這裡接著講同一個 CFF 的**另一面向——餌的色彩與寫實度該怎麼選**，並配合水色：
