...
> 2. `3A_Findings` — 8–15 條，每條以 `V3A-01`、`V3A-02`… 格式編號，每條必須含量化數值，**嚴禁模糊描述（高、低、強、弱）**
> 3. `Carry_Forward_To_3B` — 高壓舊魚心理機制參數（供卷 3B 引用）
...
**優先情況（已上傳上游報告）：**
- 已上傳 `0D 報告`：引用 `Baseline_Facts`（B0-*）與 `Waterbody_Model_Table`（管理池壓力條件、水溫），標注引用編號
- 已上傳 `1A 報告`：引用光觸發閾值（lux）、氣壓反應（hPa）、溫差閾值（°C）（V1A-*）
- 已上傳 `2A 報告`：引用 OFT 能效公式（E_gain/E_cost 比）、Reaction Strike 觸發速度（m/s）、皮質醇印記機制（V2A-*）
- 已上傳 `2B 報告`：引用側線振動頻率響應（Hz）、近場/遠場感知臨界距離（cm）（V2B-*）
- 本卷只處理高壓舊魚的心理/神經/感官機制；戰術配方轉發至 3B

**Fallback 情況（未上傳上游報告）：**
若未提供上游文件，先列出 `Missing_Upstream_Context`，再以保守假設完成分析（結論信心等級降為「中」）：

**六大水體精簡 fallback（聚焦高壓壓力背景，詳細情境見第五章）：**
1. 北部野生埤塘：水深 <3m；極育土褐色水，能見度 30–50 cm；低捕撈壓力
2. 北部深水埤塘/水庫（如石門、寶山）：水深 >10m；極育土；能見度 100–150 cm；低壓
3. 北部管理池：水深 <2m；高捕撈壓力；水車/增氧機非常態開啟（僅高溫低風時段啟動節電，平時關閉）；新舊魚二元明顯
4. 南部野生埤塘：水深 <2m；弱育土；低壓；夏季 >32°C 高溫覓食停止
5. 南部深水埤塘/水庫（如曾文）：水深 >10m；弱育土；低壓；旱季消落帶棲位壓縮
6. 南部管理池：水深 <2m；弱育土；高捕撈壓力；停機期高溫易轉硫酸鹽還原，產生 H₂S 與黑色 FeS 沉澱
...
以下主題**嚴格排除**：
- ❌ 具體戰術配方（假餌型號、操作動作序列）——那是卷 3B 的任務（本卷機制分析後以精簡映射表呈現作釣涵義）
- ❌ 重寫 1A/2A/2B 底層感官機制正文——只引用發現編號
- ❌ 繁衍期護巢防禦性攻擊——那是卷 4B 的任務
- ❌ 低壓野生埤塘一般覓食行為——超出本卷高壓場景範疇（若提及，標示為對比背景）
...
### `3A_Findings`
8–15 條，每條附唯一編號（格式 `V3A-01`、`V3A-02`…）。必須涵蓋：Mid-Strolling CFF 頻率估算（Hz）、Follower Rejection 急停窗口（s）與失焦臨界距離（cm）、Alert Reset Time 皮質醇半衰期（hr）及南部夏季/北部冬季最小投擲間隔（min）、新魚/舊魚皮質醇二元分離門檻（ng/mL）。每條格式：
> [V3A-XX]：[發現描述，含量化數值/估算區間]。[信心等級：高/中/低]。[若為推測，標示理論基礎]

### `Carry_Forward_To_3B`
列出卷 3B 極端情境推演應直接繼承的參數。每條需明確指出：(a) 機制類型、(b) 量化條件/閾值、(c) 適用水體/情境/Zone-A/B/C、(d) 引用的 V3A-XX 編號。涵蓋：
...
列出本卷研究後仍無法解決的缺口，說明：缺失什麼數據、影響哪些下游卷（3B）、建議的後續研究方向。
