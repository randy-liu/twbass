...
GLOSSARY = [
    ...
    ("ph",                  "pH",               "",             "酸鹼值（0–14），中性 = 7。台灣北部水體偏酸（pH 5.5–6.5），南部偏鹼（pH 7–8）。影響化學物質溶解度與魚類感知門檻。"),
    ...
    ("lux",                 "lux",              "照度",         "照度單位（勒克斯），測量光到達表面的強度（1 lux = 每平方公尺 1 流明）。晴天戶外可達 100,000 lux；滿月夜地表約 0.3 lux；魚類褪黑激素受光抑制的門檻僅 0.01 lux。月光在透光清水（如深水水庫冷季）1.5 m 深仍保有 0.039 lux（超過門檻，魚被「誤判為白天」繼續活躍）；染色或濁水（高 CDOM 茶色水、爆藻綠水、管理池）同深度僅剩 0.0005 lux（低於門檻，月光失效）。攻擊上衝時，魚從 1.5 m 至 0.5 m 光強可暴增 70 倍（17→1197 lux），導致閃光致盲而縮回。"),
    ...
    ("cdom",               "CDOM",             "有色溶解有機物", "有色溶解有機物（Colored Dissolved Organic Matter）。由有機質分解產生的棕褐色水溶性有機化合物，主要成分為<strong>腐植酸（humic acids）</strong>與<strong>黃腐酸（fulvic acids）</strong>。<br><br><strong>逐口看水色，不是乾淨的南北差</strong><br>一口池子 CDOM 高不高，主要看周邊落葉/植被、水源與換水快慢（滯水越久越濃），逐口不同；紅土給的是礦物「濁」、不是 CDOM。現場用<strong>水色</strong>判：茶褐＝高、綠濁＝藻、偏清＝低。<br><br><strong>與 Schreckstoff 的交互作用</strong><ul class=\"gl-list-inner\"><li>茶色 CDOM 像保護套、又遮 UV → 警報素分解慢、撐得久</li><li>偏酸＋黏土多＋日照足 → 警報素快速絮凝沉降、光解</li></ul><strong>死區傾向（看水色＋pH，無乾淨定量）</strong><ul class=\"gl-list-inner\"><li>中性/茶色/滯水場（南部較常見）：死區較大、恢復較慢</li><li>酸性/清水/日照足場（北部較常見）：死區較小、恢復較快</li></ul>中魚或跑魚後換點 ≥8 m 最保險，以現場水色為準。"),
    ...
    ("cff",                 "CFF",              "臨界閃爍融合頻率", "臨界閃爍融合頻率（Critical Flicker Fusion），單位 Hz——魚眼的「取樣速度／畫面更新率」：餌的抖動/擺尾頻率一旦<strong>超過當下 CFF</strong>，魚眼就來不及把「一下一下」分開看，那串快速抖動會被<strong>抹平成平穩、看不出在抖的樣子</strong>（就像電風扇轉快了，扇葉明明在轉卻看成一個平穩圓盤——消失的是「葉片快速掃動」，不是扇子停了）。<br><br>注意：被抹平的是餌的<strong>快速抖動</strong>，不是餌整顆凍住——餌還在、還在往前移動，只是<strong>看不出在抖</strong>、不像活餌，於是誘咬失效。這發生在<strong>抖動快過 CFF（即 CFF 偏低：黃昏/夜間/低溫）</strong>時，所以對策是放慢、換低頻軟餌，把抖動拉回 CFF 以內。<br><br>CFF 受<strong>水溫</strong>與<strong>光線</strong>兩軸控制：<br><strong>白天（明視覺）隨水溫呈倒 U</strong>：暖水約 30°C 最高 <strong>62–68 Hz</strong>、常溫 22°C 約 <strong>30–60 Hz</strong>、冷水 15°C 降到 <strong>35–42 Hz</strong>、極端高溫 35°C 因熱緊迫反降到 <strong>38–45 Hz</strong>。<br><strong>一入夜／黃昏弱光（暗視覺）</strong>：不論水溫直接暴跌到 <strong>5–15 Hz</strong>（峰值約 10）。"),
    ...
    ("schreckstoff",        "Schreckstoff",     "",             "受傷魚釋放的水溶性警報化學物質。觸發門檻極低（10⁻¹⁸ 稀釋）。死區大小看<strong>水色＋酸鹼</strong>：中性/茶色/滯水（CDOM 保護、遮 UV）撐得久、死區較大（南部較常見）；偏酸/清水/日照足則快速絮凝光解、死區較小（北部較常見）。逐口不同、無乾淨定量，中魚/跑魚後換點 ≥8 m。"),
    ...
    ("lc50",                "LC₅₀",             "半致死濃度",   "半致死濃度（Lethal Concentration 50%）：在標準測試條件下，使受試族群中 <strong>50%</strong> 個體死亡的水中化學物質濃度。<br><br><strong>標準測試規格</strong><ul class=\"gl-list-inner\"><li>暴露時間：<strong>96 小時</strong>（96-h LC₅₀，硬骨魚類急性毒性標準試驗）</li><li>測試條件：靜止水體、控溫、無食物</li><li>結果單位：mg/L</li></ul><strong>大嘴黑鱸對 H₂S 的精確值</strong><ul class=\"gl-list-inner\"><li>96-h LC₅₀ = <strong>0.0297–0.0316 mg/L</strong>（ASTM 標準測試，精確測量值）</li><li>台灣南部爛底水體底層 H₂S 峰值：<strong>8.5 mg/L</strong> → 超出 LC₅₀ 約 <strong>270 倍</strong></li></ul>⚠️ LC₅₀ 是「50% 會死」的濃度，不是安全門檻。行為迴避門檻（魚開始主動逃離）約為 <strong>0.002 mg/L</strong>，已比 LC₅₀ 低一個數量級以上。"),
    # ── 新版（初/中/專）補充術語 ──────────────────────────────────────────
    ...
    ("wind-driven-current", "風生流",           "",            "風推動表層水往<strong>下風岸</strong>堆積形成的水流，把浮游與小型獵物集中到下風岸，魚跟去結構後方逆流伏擊——<strong>下風岸常是黃金帶</strong>。怎麼找：看水面被風聚成一條的泡沫、浮渣、漂浮物（「泡沫線」，常積在下風岸邊）就是食物濃縮處。風太大時魚退入結構後方靜水區定點等，把餌慢慢送進那個靜水角落。"),
