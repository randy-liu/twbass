...
**A. 最高優先——回退風險（PATCH 已修、instruction 未回寫，重跑前必改）：**
- **FIX-REL-A1｜警報素/Schreckstoff 殘留**：2026-07-01 條目已將 club-cell 警報素死區「全庫移除」並稱同步 SUP-B/SUP-C instruction，但實際殘留兩處未清——`3A：…-instruction.md` 六、Q3-3（第 137 行「鉤中逃竄是否觸發 Alarm Substance（皮膚傷害素）擴散？靜水擴散半徑 m 與稀釋半衰期 min」，3A instruction 未在該次同步範圍內）；`SUP-B：…-instruction.md` YAML `Key_Mechanisms`（第 6 行「腐植酸-警報物質大分子錯合（Complexation）」中文殘留）。黑鱸（Perciformes/Centrarchidae）無骨鰾類 club-cell 系統。**動作**：兩處刪除（3A 該問可改為視覺/水波驚擾；SUP-B 已聚焦 Fe/H₂S，清掉殘留框架即可）。
- **FIX-REL-A2｜水體 pH 4.5–5.5 誤植**：2026-06-25「全卷 pH 框架校正」已把白皮書/術語表改為常態池水 5.5–6.5、註明 pH 4.5 僅春雨酸徑流瞬時事件值、土壤 ~5.3–5.5、土酸≠水酸；但 instruction 仍以 4.5–5.5 為常態水——`0B`（第 68/104 行，土壤 pH 正確但被當水柱絮凝條件輸出）、`0C`（第 82/154 行，酸性絮凝沉降）、`2A`（第 85 行，腐植酸穩定水色）。反例：`4A`（5.5–6.5）與 `SUP-B`（5.5–6.8）已正確——證明修正做一半、未全庫傳開。**動作**：0B/0C/2A 常態水改 5.5–6.5、明標土/水分離與 4.5 為事件瞬時值；絮凝/暴雨澄清速度（B0-06）連帶下修（絮凝模型內的事件 4.5 得保留）。

**A 已執行（2026-07-02，逐條 Edit）：**
- A1：`3A`:137 改為明列「黑鱸為 Perciformes/Centrarchidae、無骨鰾類 club-cell 警報素系統」的負向防護閘，並把該問改為量化視覺／水波-側線驚擾傳遞（去 Alarm Substance 擴散半徑/半衰期）；`SUP-B` YAML `Key_Mechanisms` 與 §三角色去「腐植酸-警報物質大分子錯合」、改為 Fick 穩態擴散/Fe²⁺·H₂S 毒理。
- A2：`0B`（§四 fallback＋§五對比表）、`0C`（§四 fallback＋六 C1 沉降）、`2A`（§四水色 fallback）常態水 pH 改 5.5–6.5，加「土酸≠水酸、pH 4.5 為春雨酸徑流瞬時事件值」註；絮凝模型的事件 pH 4.5（0B ζ電位比較、0C 事件澄清）保留。
- 驗證（grep *instruction*.md）：Schreckstoff／警報物質殘留＝0（3A 僅存負向防護閘）；常態「pH 4.5–5.5」表述＝0。**B0-06 澄清速度的實際下修待報告重跑時落實**（instruction 已標「事件期快速澄清 vs 常態緩慢沉降」兩情形）。

**B. 系統性（架構層，跨全卷，換模型不會好）：**
- **FIX-REL-B1**：全卷「嚴禁模糊描述＋所有關鍵參數必給數值」× 黑鱸近零物種實測 = 結構性誘導幻覺；四層信心標注是防線，但下游把第 3–4 層（廣泛外推/理論估算）洗成確定數字經 Carry_Forward 餵下游。**動作**：允許「無可靠估計/依個體」為合法輸出；指定第 3–4 層者禁進 Carry_Forward 當定量基準。
- **FIX-REL-B2**：SUP 卷 Correction_Instructions 用外推「[確認]」下游外推＝循環自我權威化。**動作**：Correction_Instructions 標其依據信心層級，理論估算不得用「[確認]」字樣。附：`SUP-D-A` 第 153 行 NTU 感官矩陣明令「不可留空或填『視情況而定』」——直接禁掉正確 hedge，必刪。

**B 已執行（2026-07-02，逐條 Edit）：** 於 `TEMPLATE`＋9 主卷（1A/1B/2A/2B/2C/3A/3B/4A/4B）＋7 SUP 卷（SUP-A/B/C/D-A/D-B/D-C/E）§一「反幻覺原則」四層框架後，新增「誠實輸出優先於格式」bullet——(a) 查無依據者允許輸出「無可靠估計／依個體而定」、不得虛構精確（B1）；(b) 第 3–4 層外推值禁入 `Carry_Forward` 當下游硬基準、信心標「低」（B1b）；(c) SUP 卷另加「[確認]」僅限第 1–2 層證據、外推改標「[方向性支持・信心低]／[待實證]」（B2）。另修 `SUP-D-A` 禁 hedge 矩陣（line 153「不可填視情況而定」→ 允許「不確定／依個體而定」）與 NTU 硬性要求。**設計取向**：不放寬量化、不回頭寫「高/低/強/弱」，只加合法誠實出口＋下游信心閘。0 系列（0A–0D，物理/地化紮實、屬 E 類）未納入。驗證（grep）：17 檔各含 1 條新 bullet。

**C. 跨物種／神經術語污染（點狀，易改）：**
- **FIX-REL-C1｜2B**：Q2 內耳聽石聽覺問題缺防護閘。**動作**：於 §一或 Q2 明列「黑鱸為聽覺泛化種（hearing generalist）、無韋伯氏器、聽覺上限約 1 kHz、最敏 100–400 Hz，禁引骨鰾類（鯉/鯰）聽覺數據外推」。
- **FIX-REL-C2｜2A**：第 121 行「杏仁核」為哺乳類構造→改魚類端腦內側區；第 128 行「Superior Colliculus（上丘）」→改視頂蓋（optic tectum）。
- **FIX-REL-C3｜4B**：蛇頭魚學名掛錯——現況「魚虎=*Channa argus*、泰國鱧=*C. micropeltes*」，正確為魚虎=小盾鱧 *C. micropeltes*、泰國鱧=腺鱧（兩者皆大型強勢外來種）；七星鱧（*C. asiatica*）/斑鱧（*C. maculata*）體型較小較溫馴、勿混。
- **FIX-REL-C4**：`SUP-D-A` 把化學感當高濁度共主導覓食通道（黑鱸化學感中等、非鯰魚級，高估）→降輔助；`SUP-E` 第 206 行側線辨識距離拉到 120 cm（超近場 ~<80 cm）→>80 cm 改視覺主導；`SUP-E` 第 153 行混入「鯰魚側線」與黑鱸卷無關→刪。

**C 已執行（2026-07-02，逐條 Edit）：**
- C1 `2B`：Q2 內耳聽石前加物種防護閘（黑鱸為聽覺泛化種、無韋伯氏器、上限約 1 kHz、最敏 100–400 Hz、禁引骨鰾類聽覺數據外推）。
- C2 `2A`：杏仁核→端腦內側區（medial pallium 同源）；Superior Colliculus→視頂蓋 optic tectum、視覺皮層→端腦視覺整合區。
- C3 `4B`：蛇頭魚學名更正（魚虎=小盾鱧 *C. micropeltes*、泰國鱧=腺鱧；七星鱧/斑鱧為較小溫馴種），並依使用者現場知識**大幅擴充**——(a) **全體型捕食**：小盾鱧約 100 cm、腺鱧約 75 cm 均大於台灣野生黑鱸上限約 55 cm，捕食含成魚之各體型、單次攻擊即致死，壓抑補充量＋成魚存活率；(b) **管理池排除**：池主清除大型鱧，鱧害僅算野生／半野生水體；(c) **暖化三區分布**：腺鱧遍布全台（都市暖水池尤然）、小盾鱧中部以北難過冬（主要中南部：日月潭、白河、曾文，已有筏釣服務）。改及 §五 bullet／競爭表、Q3、Metadata。
- C4 `SUP-D-A`：>200 NTU 感官矩陣「側線+化學」→「側線主導（化學僅近距輔助）」；`SUP-E`：水蛇側線辨識 40–120 cm→近場 40–80 cm（>80 cm 改視覺）、刪 line 153 混入之「鯰魚側線」。

**D. 偽精確／偽機制（各卷 mode C/D；降級「理論估算」或放寬 hedge、不進 Carry_Forward 硬基準）：**
- SUP-A：CFF↔操餌速度偽定量鏈（移動假餌無明確閃爍頻率；釣手控速度非精確 Hz）、慢性皮質醇 ART 覆蓋 3B（自承缺現場血檢卻輸出具體 ng/mL＋ART 修正）。
- 3A：Mid-Strolling「視覺催眠/CFF 神經同步」為發明機制、Follower Rejection「近距失焦反向觸發」失焦臨界 cm、新<15/舊>30 ng/mL 二元門檻當 3B 分策硬依據。
- 3B：颱風前氣壓「爆發性覓食」（physoclist 精確氣壓感知證據薄弱）、H₂S↔皮質醇量化相關式。
- 1A：氣壓魚鰾「各機制貢獻%」、月相 melatonin 抑制%＋日間低迷 hr/day。
- 1B：第 116 行「4°C 最大密度翻水」——亞熱帶淺塘到不了 4°C，翻水機制實為表水冷卻＋風混合去分層。
- 4A：精子 pH 5.5/6.0/6.5/7.0 逐級游速 μm/s（近中性效應輕微卻逐級量化）。
- 4B：Q4-2 護巢 vs 覓食攻擊「fMRI 神經迴路」（黑鱸此類 fMRI 幾不存在）、Lotka-Volterra α/K 值/捕食成功率零台灣數據全外推。
- SUP-C：冒險覓食「單次衝入吸收劑量 µg/kg、單日安全次數」（自承可能無 M. salmoides 紀錄仍強制輸出）。
- SUP-D-C：生物漂移 μm/s＋搜索映像 LTP ms＋端腦凌駕皮質醇 30 ng/mL。

**D 已執行（2026-07-02，逐條 Edit）：** 實質機制錯改寫＋偽精確軟化——
- `1B`：4°C 翻水改**非武斷版**（亞熱帶淺塘常態遠高於 4°C、主靠冷鋒降溫至表底同溫＋強北風混合去分層；近 40 年不到 5 次超強寒流氣溫 <4°C、淺塘水溫因熱慣性更高，極端逼近 4°C 反轉屬罕見理論方向）。
- `3A`：Mid-Strolling「視覺催眠/CFF 神經同步」→ 行為觀察＋未證實假說標註（Metadata 去「視覺催眠」）；Q1 改「三分」；Follower Rejection 失焦臨界 → 未證實假說、不進 Carry_Forward。
- `SUP-A`＋`2B`：CFF-操餌偽鏈 → **四訊號拆解表**（①平移→motion blur／②擺動動作頻率→側線 2B／③反光閃爍→CFF／④咬→行為）＋假餌頻譜歸位（paddle tail 低頻 thump、lipless 高頻緊震、spinner 反光）；SUP-A Metadata 擴為「視覺時間解析（動態模糊＋CFF）」；2B Q6 加分流界定。（此段依使用者多輪討論深化，超出原 FIX 範圍。）
- `4B`：護巢 fMRI → 行為區辨（黑鱸無此 fMRI、標理論方向）。
- `3B`：颱風前氣壓「爆發性覓食」→ 釣界觀察/生理未證實、不要求 meals/day 量化；H₂S↔皮質醇相關式 → 禁具體相關式、改定性。
- `1A`：氣壓「各機制貢獻%」→ 只比證據強弱、不給 %；月相 melatonin 抑制 %＋hr/day → 定性、信心低。
- `4A`：精子 pH 逐級 μm/s → 定性（5.5–6.5 近中性、效應輕微，不逐級硬量化）。
- 未單獨改（交 B 的誠實出口＋CI 信心閘覆蓋）：3A 新/舊魚皮質醇二元門檻（已標「估算」）、SUP-C 冒險劑量、SUP-D-C μm/s/LTP（Unresolved 已誠實列缺口）。

**E. 相對乾淨、可近原樣重跑：** 0A（純氣象，最安全）、0D（整合，僅繼承 pH）、0B/0C 物理化學部分、SUP-C（設計最佳，主動查證溯源，可當範本）、SUP-E natural history（在地圖鑑錨定紮實）。

**建議執行序**：A（必改，定點刪改）→ 拍板 B（是否放寬量化強制，換模型前最該定）→ C/D 批次修。母版 0–4 若要與 split 同步可一併掃改。**下一步交付選項**：本條為選項(1)登錄清單；選項(2)為直接動手改 instruction（建議先做 A 兩類回退風險）。
...
緣由：使用者用「SD 1 m、魚在 3 m、綠水」情境指出——顏色雖權重低，但是**偵測閘門**（選錯色→反射不出光→偵測不到→咬餌率掉），且釣手會推錯（綠水選綠/把茶褐水「紅穿遠」誤套綠水）。需要一張查得到的決策表擋掉這類錯。並更正使用者一處物理：綠水中紅光被葉綠素吸收、不會「走更遠」（那是茶褐水規則）。

- §20 把原三條文字 bullet（濁褐/綠藻/清水）改成 **4 列決策表**：欄＝水色（怎麼染的）｜哪段光穿得進＝看得到｜該選餌色｜為什麼+現場修正。①茶褐/丹寧（CDOM 吃藍→紅橙穿最深，紅最先消失只清水成立）；②綠藻（葉綠素吃紅+藍→純紅死灰嚴禁、純綠跟背景融合→用更亮螢光黃綠）；③清水（自然低調、別過鮮 lure-shyness）；④**任何水色但光照不夠**（深>約 2–3 倍能見度 或 <15cm）→ 純黑剪影＋強振動、色彩失效、接 §十（直接收使用者 SD1m/魚3m/光剩~0.6% 情境）。
- 表前補兩條使用提醒：顏色只負責偵測（咬不咬看尺寸/動作/輪廓）、定性非逐 cm 精算（避免偽精確）。藍尾錨等清水視覺錨移到表後一句保留。
- 未動 H2/H3，TOC 維持 30。重 build。

---

## senior §20 顏色框架整合（波長＋顏色=能見度非身分）＋擬真在地註解＋藍尾錨對齊（2026-06-29）

**性質**：觀念校正＋可讀性（依使用者連續追問：波長、色覺、能見度目的、藍尾錨）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`SUP-E_….md`、`台灣淡水與沿岸水生爬蟲類獵物報告.md`、`docs/index.html`

緣由：使用者指出 §20「濁褐水用深紅」與「紅最先消失」矛盾，並追問顏色對二色覺黑鱸到底是不是辨識/拒咬線索。回溯 2A V2A-08/09/10（波長衰減）、2C（614/535 二色覺）、SUP-D-A line 66（尺寸對>顏色對 3.5 倍、顏色僅 <30cm 弱細節）、2A line 139（lure-shyness 編入顏色為弱特徵）後確認：方向需整合校正。

- **§20「顏色怎麼配水色」整段重寫**，四點：①**波長修正**——褐水 CDOM 吃藍、紅/橙穿最深（「紅最先消失」只清水成立、褐水相反）；綠藻水葉綠素吃紅、純紅變死灰禁用、用黃綠；剪影（黑/暗深藍）與反射色（波長匹配）兩機制分開。②**顏色=能見度/對比、非身分**——黑鱸紅綠二色覺、顏色僅 <30cm 弱細節，**不因顏色「不像真食物」拒咬**（螢光黃/泡泡糖粉照中為證）；咬不咬看尺寸/動作/輪廓。③**能見度要「剛好夠」非越鮮越好**——濁水偵測瓶頸拉對比、清水收斂自然低調（高壓老魚 lure-shyness 會把鮮豔顯眼連到被釣痛，呼應 §九 finesse）。④**藍尾錨對比化**——450–475nm 靠清水中高亮度/高對比、非黑鱸藍色覺，且僅清水有效、褐水失效。
- **§八 L188／結語 L491「擬真」在地註解**：標明「擬真＝尺寸/動作/頻率對上、非塗裝；魚不審顏色真假」，指向 §二十（§九/§十二/§371 的擬真本即釘成尺寸/動作/質地，未動）。
- **上游藍尾對齊**：SUP-E VSUP-E17、爬蟲報告麗紋石龍子段各加「機制註」——錨靠亮度/對比非藍色覺、清水限定、褐水失效（不重寫原描述）。
- 未動 H2/H3，TOC 維持 30。重 build。

---

## senior §19／§21 兩張頻率表改清楚（加假餌欄、獵物↔餌對齊）（2026-06-29）

**性質**：可讀性／可操作性改寫（描述放完整、不省字，依使用者要求）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

緣由：使用者指兩張表「告訴你獵物訊號、卻沒直接告訴你綁什麼怎麼操」，查表當下不夠直接。

- **§19 頻率指紋表**：原 4 欄（獵物動作｜頻率｜主感應器｜觸發攻擊類型）格式雜、無假餌欄。改為「獵物動作（發出的訊號）｜頻率（哪個感官在收，SNs/CNs/聽石含白話註）｜**對應假餌（選餌＝選頻段）**｜怎麼操作、觸發哪種咬」；補表前「讀法」說明。每列描述放完整（保留剪切波擴散、噴射流瞬速等物理細節，再補綁什麼餌＋怎麼操）。
- **§21 物候映射表**：第三欄原把多種餌用分號擠成一坨、獵物↔餌配對不清。改用 `<br>` 逐項換行、每隻獵物用「→」接它各自的餌與操作；破防窗口（白蟻分飛／蝦蟹軟殼潮／颱風後脈衝）標粗體；補表前「讀法」說明。鱉苗補上對應餌（沙褐圓盤軟蟲底層慢爬）。
- 表格頻率數值、感官分工、物候時序均未改動（僅重排＋補假餌欄＋補述）。未動 H2/H3，TOC 維持 30。重 build，`<br>` 於 cell 正確渲染。

---

## senior §九 CFF 段重組瘦身＋§二十 補「寫實度」半段（2026-06-29）

**性質**：可讀性重組（更白話、字數不增反減）＋觀念補強。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

緣由：使用者讀 §九 撞到「CFF 高→打反射跳過視覺、CFF 低→放慢讓魚看清」的矛盾感。核對 §十/§十一 後確認**白皮書整體沒破**（側線反射、軟硬正交都已在 §十/§十一），矛盾來自 §九 局部把低 CFF 寫成視覺口吻「讓魚看清」、又沒指路 §十。修法是重組＋砍、不是大重寫（大重寫只會更長更不白話）。

- **§九 CFF 段（line 210–226）重組瘦身**：~17 段密文壓成 9 段。砍掉獨立 rubber jig 整段（其完整版在 §十九，§九 改為「見 §十九」一句帶過）、把「夏季正午越快越炸」併入「天花板高」、⚠️框壓短。核心修正：低 CFF 不再寫「讓魚看清」，改為**反射不消失、只是從視覺換到側線通道（指路 §十）；「放慢」是為餵進側線圈＋冷水代謝慢**。高 CFF 講清「生口→尺寸/動作擬真乾淨吃、老魚→抖停加速」。錨點 `senior-cff`、§十/§十九/§二十 跳轉全保留。
- **§二十 補「寫實度」半段**：§二十 引言本就承諾「色彩**與寫實度**該怎麼選」但只寫了色彩。補：①「寫實」對魚＝尺寸＞動作＞剪影（見 §十六），**塗裝細節幾乎不影響**，但**顏色仍重要**（角色是配水色讓魚看得到/對比夠，非騙過細審——避免與下文水色選色矛盾）；②硬餌＝動作觸發（泳姿即反射、照片級寫實是做給人看的）、軟餌＝近距細審存活（質地/微動/free fall）。不重複 §十六（拒咬權重）、§十一（軟硬正交）、§十九（選餌頻率）。
- 全 senior 淨減 ~1110 字元（§九 砍掉 > §二十 補入）。未動 H2/H3，TOC 維持 30。重 build。

---

## 1B 風生流「迎風岸／下風岸」用詞顛倒扶正（2026-06-29）

**性質**：上游報告用詞錯誤更正（物理方向）。**目標檔案**：`1B_六大水域棲位模型與風生流.md`、`1B：六大水域棲位模型與風生流-instruction.md`

緣由：白皮書數值出處深掃時發現，1B 報告把風生流的岸別標反了。物理真相：風吹表層水順風走，在**下風岸**（風吹去、浪打到那側）堆積下沉（Downwelling）、漂浮物與浮游生物聚集；**迎風岸**（風來那側）表層被吹走、底層補償上湧（Upwelling）。1B 報告卻把 Downwelling＋食物聚集＋風浪拍打那岸稱「迎風岸」、Upwelling 那岸稱「下風岸」，整組顛倒。白皮書三版講「下風岸（風吹過去、浪打到那側）」物理正確、未受影響；錯誤孤立在 1B（其 instruction L137–141 結論本即正確、僅 L136 括號同樣顛倒）。

- **1B 報告 V1B-13／V1B-CF-07**：流場結構岸別對調（下風岸=Downwelling、迎風岸=Upwelling）；「食物集中／餌料魚富集／伏擊帶」全部由「迎風岸」更正為「下風岸」（L141/142/145/146/178）。
- **1B instruction L136**：括號 `Downwelling，迎風岸`／`Upwelling compensation，下風岸` 對調為 `Downwelling，下風岸`／`Upwelling compensation，迎風岸`。
- 白皮書三版與其他卷未引用此錯誤（皆用下風岸），無需改；不涉 build.py／index.html。

---

## 數值出處核查（三）：補充損耗 >70% 與皮質醇「實測」過度斷言修正（2026-06-26）

**性質**：出處核查＋過度斷言軟化。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`-expert.md`、`docs/index.html`

緣由：對白皮書承重數字做系統性回溯上游（抽查最承重一批）。大多有扎實出處（體積熱容 0B V0B-12、OFT 2A V2A-01/02、CFF 3A V3A-02／SUP-A、致盲 1A V1A-10、打巢 4B、反射 2A/SUP-D-B、避毒高度 SUP-B VSUP-B11／4B），但抓到兩個過度斷言：
1. **「族群年均補充損耗 >70%」**（senior §十八、expert 第七部）：4A/4B 只有分項折損率（棄巢率 65–95%、無雄魚存活近零、K_eff 壓縮 90%、外來種北美 5–10 倍），**查無此聚合值**；且前一輪已軟化 §十七 同類「血盤式 >70% 斷言」、漏了這兩處。→ 改「補充量嚴重折損／受損」定性。
2. **皮質醇「台灣計費池實測 35–45 ng/mL」**（senior §十二，2 處）：SUP-A〈VSUP-A06〉自承「35–45 來自**飼養實驗室研究**、台灣現場血液採樣**付之闕如**」。「實測」→「推估」。

未動：體積熱容「北部 3.411」collapse 三區（3.411 為 Zone-B 紅壤值代表北部、Zone-A 實為 3.215）——屬簡化非捏造，依指示不動。重 build，TOC 16/30/20 不變。

---

## Schreckstoff×CDOM 假精確全面移除＋改定性（SUP-B→3A/SUP-C/白皮書/術語表）（2026-06-26）

**性質**：移除無實測支撐的數值推演、改定性（依「無實測之數字計算/推演不保留」原則）。**目標檔案**：`SUP-B_….md`、`3A_….md`、`SUP-C_….md`、`台灣大嘴黑鱸白皮書-senior.md`、`-junior.md`、`-expert.md`、`build.py`、`docs/index.html`

緣由（為何移除——正文一律不留此說明，全寫在這）：審查 senior §三 vs §十四 的 CDOM 南北方向矛盾，回溯 SUP-B 發現整套「Schreckstoff×CDOM 錯合」死區模型根基有三個問題：
1. **假繼承**：區域 DOC（北 12／南 3 mg/L）列於「引用自 0B」名下，但 0B 僅有底泥有機碳、**無水體 CDOM 數據**——實為 SUP-B 自設、掛了 0B 的出處。
2. **方向書內矛盾**：SUP-B＋senior §三 隱含北高南低、術語表＋senior §十四＋2026-06-25 決定為南高北低；死區大小結論一致但 CDOM 方向與角色相反。
3. **死區半徑（3.35/3.90/4.91 m）、活性比例（58/73/94%）、恢復分鐘（北 10–15／南 30–45）全由上述假繼承＋類比常數推算、無台灣實測**；CDOM 本質為逐口池子變數（落葉/植被/水源/滯水），不該套乾淨南北標籤。

處理（CDOM 來源、機制、看水色判高低等「知識」保留；數值計算/推演移除）：
- **SUP-B §1（VSUP-B01–B06）**：移除 DOC／$K_{doc}$／$K_a$／活性%／死區半徑／恢復時間／光解半衰期，改定性（機制＋「死區看水色＋pH、換點 ≥8 m」）。Core_Parameters、Inherited（刪假繼承的水體 CDOM、留 pH 5.5–6.8）、兩個 Carry_Forward、給 3A 的 Correction_Instruction、Unresolved 同步改定性。**Fe²⁺（§2）與 H₂S（§3，避毒高度 100/150 cm、水車重啟）完全未動。**
- **senior §十四**：拿掉三區半徑＋恢復分鐘＋94%，改「看水色＋酸鹼：中性/茶色/滯水死區較大恢復較慢、酸/清水/日照足較小較快；CDOM 逐口看水色、能掛南北的是 pH」。§329 的 45–90 min／系統性閉口 12hr·36–42hr 為 3A ART 數（有出處）、保留。
- **junior §四**「影響範圍約 4–7 公尺」→「影響好幾公尺」；**expert 劇本二**「死區近 5 m、活性 94%」→「南部中性水死區較廣、撐較久」。
- **術語表 CDOM／Schreckstoff 兩條**：去掉「南部高 CDOM／北部低 CDOM」與 <0.5 m／4–7 m 半徑，改「逐口看水色＋pH、換點 ≥8 m」。
- **SUP-C** 引 VSUP-B05/B06 改定性；**3A** 局部驚嚇「2.5–4.0 m 待 SUP-B 精算」→「中魚點附近一小圈、換點 ≥8 m」。
- 重 build，TOC 16/30/20 不變。

註：本條部分**取代**下一條（2026-06-26 一致性修復）對 ③ 警報素「半徑改範圍約、數值不動」之處理；該條維持原記錄不改寫，以本條為最新。

---

## 三版跨版本一致性修復＋軟殼 OFT 倍數 erratum（2026-06-26）

**性質**：跨版本數值一致性校正＋上游 erratum（每條矛盾逐一回溯原始研究報告核對後扶正）。**目標檔案**：`台灣大嘴黑鱸白皮書-junior.md`、`台灣大嘴黑鱸白皮書-senior.md`、`台灣大嘴黑鱸白皮書-expert.md`、`SUP-E_…月曆與假餌映射.md`

緣由：審查 junior/senior/expert 三版時抓出五處跨版本數字/結論不一致，逐條回上游報告核對。其中三處查出非真矛盾（各有出處、量級或情境不同），兩處需扶正；另查出軟殼 OFT 倍數為上游 SUP-E 報告括號抄錯。

- **連投間隔（扶正）**：3A〈V3A-13〉＋SUP-B〈VSUP-B06〉一致為「北 10–15／南 3–5 min」、溫度驅動。senior 正文「北 10–20」與同節精要「北 15–20」（兩處自相矛盾）、junior「北 10–20」全部扶正為 **10–15**；expert 劇本一（北部管理池·**盛夏**）誤用冷水值 10–15，改 **≥3–5 min**（盛夏高溫 ART 短、同南部盛夏）。
- **溫躍層強度（非真矛盾，補情境標註）**：senior §十「−5°C/m」實為 2B〈V2B-07〉南部曾文局部極端分層、expert 1.5–2.5 為 0C 北部石門典型，各有出處。senior 改標「北部水庫典型 1.5–2.5°C/m、南部曾文局部極端可達 5°C/m」消除「相差兩倍」誤會，數值不動。
- **警報素範圍（非真矛盾，僅字眼）**：junior「影響半徑 4–7 m」實為 SUP-B 建議偏離距離（北 4–5／南 6–7 m）、senior 3.35/3.90/4.91 m 為死區半徑，屬不同量。junior「半徑」改「範圍約」避免與半徑混淆，數值不動。
- **鱉苗孵化月份（扶正）**：爬蟲類報告＋SUP-E 為孵化高峰 7–9 月（峰 7–8）。senior 月曆從 6–7 月移至 **7–8 月**（junior 本即 7–8、正確）。
- **軟殼 OFT 倍數 erratum（扶正＋更正上游）**：junior 5–10、senior 5–13 統一為 **7–13 倍**（＝軟殼絕對能效 0.25–0.45 ÷ 硬殼基準 0.034 kcal/s，與 2A〈VSUP-E06〉一致）。**根因**：SUP-E 報告 L264 括號把甲殼類報告的「E/Th 處理時間倍數 5–10」誤標成 E/T 全 OFT 倍數（甲殼報告 L13 本即標明 E/Th、正確、不動）。同步更正 SUP-E L264 為「約 7–13 倍」並加註兩指標差異。
- 未動任何 H3，三層 TOC 不變（junior 16／senior 30／expert 20）。

---

## 全卷 CFF 全面校正＋頻率「靠選餌非手調」釐清＋rubber jig 雙例（2026-06-26）

**性質**：跨層數值統一＋觀念校正＋可讀性重寫（CFF 為上游少數帶直接實驗證據之參數，依 V3A-02／VSUP-A01-04）。**目標檔案**：`台灣大嘴黑鱸白皮書.md`(junior)、`台灣大嘴黑鱸白皮書-senior.md`、`台灣大嘴黑鱸白皮書-expert.md`、`build.py`、`docs/index.html`

緣由：盤點 CFF 全書出現處，發現三層數值互相矛盾（senior/expert 62–68／35–42、junior/術語表 30–60／<10），且把「黃昏」與「低溫」併在 35–42（黃昏實為暗視覺 5–15）。對照上游確認完整模型。後續使用者再戳出兩個觀念硬傷：CFF「看清」前提寫太鬆、以及「複製頻率」暗示釣手能手調 Hz（不切實際）。

- **CFF 統一為「兩軸」模型（全層）**：白天隨水溫倒 U（30°C 峰值 62–68、22°C 30–60、15°C 35–42、極端 35°C 熱緊迫反降 38–45）＋入夜/黃昏不論水溫暴跌 5–15。修正 senior §九/§二十/第六部白話、expert §視覺端、術語表（build.py）；junior 補「水溫也會推 CFF（暖水可上探 62–68）」。把「黃昏 35–42」的掛錯拆正為暗視覺 5–15。
- **術語表 CFF 重寫**：定義改「取樣速度」＋電風扇/日光燈白話；釐清「看不出在抖」是快速抖動被抹平、非物體凍住，且發生在抖動＞CFF（CFF 偏低）時。
- **人眼 vs 魚眼**：junior 6.2 修正「人眼 24–30 Hz」（那是動作流暢門檻，閃爍融合實為 50–60 Hz）；新增人 vs 魚比較表＋關鍵洞察（人溫血只隨光、魚冷血光溫雙重；寒流黃昏你視力正常但魚 CFF 已被雙殺，故判斷看水溫天色非看自己）。
- **§九 觀念校正**：① CFF「看清/慢動作」前提改為餌動作**遠低於** CFF（非勉強以內，貼近門檻就糊）；② 把「夏季越快越炸」釘上 CFF 天花板（超過即糊成拖影、不咬）；③ 明寫釣手只能控速度/加速度急停/變向＋選餌，調不出精準 Hz。
- **頻率「靠選餌非手調」釐清**：senior §十九 L435「操竿就是複製頻率」、第六部精要、速查、expert 白話框/§Match the Hatch，全部補「主要靠選餌（不同餌天生落在不同頻段）、速度節奏微調、手抖上限 3–7 Hz 調不出精準 Hz」。
- **rubber jig 雙例**：§九（CFF 濾網角度：手給慢節奏、裙擺自顫高頻，冷水低 CFF 要放慢非抖更快）、§十九（選餌角度：細軟裙顫快像蝦、粗硬裙擺慢像蟹，選裙=選頻段）。英文 flutter 一律改正體中文「顫動/細顫」。
- 未動任何 H3，三層 TOC 不變（16/30/20）。

---

## 數值定位說明＋senior §十七 重構（巢底降級、加南北水庫分野）（2026-06-25）

**性質**：可信度校準＋章節重構（依「報告皆外推、非實測」之考證結論）。**目標檔案**：`台灣大嘴黑鱸白皮書.md`(junior)、`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

緣由：實地考察連續戳出 pH、巢底、南北水庫三處與實況不符後，逐條考證 4A/4B，確認上游報告（0–4、SUP）**全為理論估算/近緣物種外推、零台灣黑鱸現場實測**，自評信心偏樂觀（V4A-11 硬底面積、V4A-02 pH→受精率已被物理與實地證偽）。結論：報告不可當地面真相，數字應定位為估算。

- **(a) 新增「數值定位說明」**：senior 導讀末、junior 開頭各加 ⚠️ 區塊——量化數字為文獻/近緣物種推估之估算、非台灣現場實測，用於抓方向與量級；與實地經驗衝突時以現場為準。junior 原僅涵蓋「基於理論推測」標註的弱免責，升級為涵蓋全部數值。
- **(b) §十七 重構**：標題改「繁殖成敗：不看巢底，看水位、水質與外來種」。① 巢底由「先天打擊」降為一句帶過（黑鱸任何硬底都築巢、小池岸邊工程硬底通常就夠，僅被逼純軟泥才低孵化；數字標估算）；② 改以四把真因為主軸——水位、水質事件（南部 H₂S；北部春雨事件且標明酸為近緣外推、黑鱸耐酸、土≠水）、外來種掠食仔魚（「孵出來也長不大」主因）、倒春寒；③ 新增**南北水庫分野**：北部水庫（石門/寶山）相對最穩繁殖據點、長年可見完整繁殖；南部水庫（曾文）乾涸＋深層 H₂S＋泰國鱧掠食→補充量趨近零、罕見成魚魚苗（對齊實地觀察與 4B 矩陣定性）。載重數字一律標估算，不出現「依 4A/4B」內部引用。
- 第五部精要同步改寫（不再以「缺巢底」開頭）。未動 H3，三層 TOC 不變（16/30/20）。

---

## 全卷 pH 框架校正＋senior §十七 繁殖段重寫（2026-06-25）

**性質**：系統性數值/框架校正＋白話重寫（依實地考察與上游 4A/0B 原始定義）。**目標檔案**：`台灣大嘴黑鱸白皮書.md`(junior)、`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

緣由：實地考察（桃園平鎮台地紅土池、水清、鱸魚正常繁殖、有吳郭魚/斑鱧）與白皮書「北部強酸→繁殖必敗」敘述明顯出入。盤點後確認為**白皮書轉寫時把瞬時/土壤值當常態、把硬底說成只有天然砂礫**的系統性誇大。

- **北部 pH 全卷校正**：junior 6 處把北部「水體 pH」寫成 4.5–5.5（L36/143/206/317/371/1053/1191），改為**常態池水 5.5–6.5**（與術語表、V4A-02 一致），並註明 pH 4.5 是**春雨酸徑流瞬時事件值**、土壤本身 ~5.3–5.5。新增「土酸≠水酸」說明（藻類光合/CDOM 緩衝拉高水 pH）。senior §濁度澄清段常態 pH 同步改 5.5–6.5（絮凝模型的事件 4.5 保留）。
- **Schreckstoff 去過度歸因**：北部失效主因改為**低 CDOM 缺保護套→快速光解**，非強酸；並修正 junior L1053 把北部誤寫「高 CDOM」之錯（北部實為**低** CDOM）。
- **senior §十七 繁殖段重寫（白話＋四點校正＋底質校正）**：① 第一重底質改為「硬底**含人工拋石/水泥岸/磚塊/根系/沉木**都能用，>90% 匱乏是**佔池底面積**問題、非沒有硬底；結構豐富的池塘繁殖正常（呼應人工產房墊）；替代硬基底孵化率 35–55%」（依 V4A-11）；② 南北殺手全框為**春雨事件型**；③ 北部黏土裹卵**清水豁免**（SS<50 mg/L→孵化 80–90%，V4A-03）；④ pH→受精率標明**近緣物種外推、黑鱸耐酸**，且**土酸≠水酸**（4A Unresolved 自承可能高估酸性危害）。補 >40 m²/100m² 單位、SS（水中懸浮固體）白話。
- 第五部精要同步軟化（先天硬底面積不足、後天春雨事件型夾殺，去掉血盤式 >70% 斷言）。術語表 pH 條目本即 5.5–6.5，未動。未動任何 H3，三層 TOC 不變（junior 16／senior 30／expert 20）。

---

## senior §十六 移除正文自問自答＋第四部精要用語同步（2026-06-24）

**性質**：文風修正（內容/數值不變）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- §十六 前一條重寫時在正文留下指涉讀者對話脈絡的自問自答（「你說的『靠近之後才判斷』就是這一步——不在第 2 步」、「別把它們混為一談」），違反「正文只寫正面事實、不留自我修正/質疑痕跡」原則。開頭改純描述（「兩個不同層次的判斷：遠處粗略快篩、嘴邊精細審查」）；第 4 步改「真正的細看發生在這一步，不在遠處的辨識」。兩層判斷的事實內容全保留。
- 第四部精要結語同步 §十五 用語：「剛被擾過的水死、久封＋飢餓的水炸」→「…死氣沉沉、…瘋狂」（前次改 §十五 漏了精要這句）。未動 H3，TOC 維持 30。

---

## senior §十六 追擊序列白話重寫＋釐清辨識/評估兩層判斷（2026-06-24）

**性質**：可讀性重寫＋邏輯釐清（數值不變）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- §十六「完整追擊序列」原為一句壓縮的五階段流程，且把「辨識」寫得像最終判定，造成「為什麼還沒接近就判斷」的邏輯疑點。重寫拆為逐階段條列並補白話。
- **釐清序列裡有兩個不同判斷**（依上游 SUP-D-B VSUP-DB07）：辨識（100–500 ms）= 遠距離、中腦、初步區分目標性質的「粗略快篩」（決定要不要游過去），評估（2–5 s）= 近距離 15–30 cm、端腦調用記憶的「精細審查」（老魚打槍主戰場）。明確「靠近之後才細看的是評估、不是辨識」。
- 否決權重（尺寸 85–95 > 振動 70–85 > 輪廓 50–65 > 化學 30–40，VSUP-DB09）、兩個破法（社交促進壓縮評估 80%・咬餌 ×2–3；Dead Drift 1–5 Hz・15–40 μm/s・門檻 10 μm/s・Shore 00-15・2–8 Hz・低 60–75%）全保留。未動 H3，TOC 維持 30。

---

## senior §十五 冒險覓食段白話重寫（2026-06-24）

**性質**：可讀性重寫（數值不變）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- §十五「毒區邊緣的冒險覓食」整段改為完整白話、不省字：開頭交代「毒區」與「要不要冒命吃」的處境；三種反應（健康魚逃 75–85%・12–18→45–110 m/hr／老魚硬撐索餌 0%／冒險覓食 3.5 倍・15–25 s 上限 30 s・45–60 min COX 冷卻）各自獨立成條講完整；皮質醇總閘門分三檔（急性 >150 封死／慢性 20–40 門檻升至 ≥6.0・降 >90%／飢餓 3–5 天 Ghrelin 短路降到 2.0–2.5 倍）。結尾兩極端用語依使用者要求改為「死氣沉沉」「瘋狂」。數值全保留，未動 H3，TOC 維持 30。

---

## senior §十三 急停決策窗口段重寫（2026-06-24）

**性質**：可讀性重寫（數值不變）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- §十三「急停決策窗口」原段把「決策窗口長短（隨水溫）」與「Dead Stop 對新老魚相反反應」兩件事混在一句，難讀。重寫拆為兩層：先講決策窗口是什麼（餌停住後魚評估所需時間，由酶反應動力學/水溫決定：夏 30°C 0.8 s／冬 15°C 2.5 s），再以條列分新魚（+30–50%，要停滿蓋過窗口、冬 2.5–3.5 s）vs 老魚（−40–60%，停=把窗口送給慢通路審查→拒咬；例外為貼結構 <12 cm 邊緣效應壓縮窗口 40–60%）。數值全保留，未動 H3，TOC 維持 30。

---

## senior §十二–§十四 術語補全＋三基線釐清＋§十四重寫（2026-06-24）

**性質**：可讀性補白＋數值勘誤回補＋術語表擴充。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`build.py`、`docs/index.html`

- **⚠️ 回補 1.68：前一條（06-23）誤判 1.68 ng/mL 無出處而刪除，實際有出處（SUP-A VSUP-A06，HVF 健康個體自然靜息基線；當時只 grep 3A 未查 SUP-A）。** §十二 改為三基線分清：健康個體 1.68±0.69（SUP-A）／剛放流 <30 天 10–15（3A）／老魚 20–40・台灣池 35–45（SUP-A）。同步修正 build.py `皮質醇`、`新鱸/老鱸` 兩術語表條目，補回放流期 10–15。
- **§十二 補白話**：GR（糖皮質激素受體）就近解釋；補兩欄位（Follower Rejection／不匹配餌反射咬率）定義。
- **§十三 就近補白話**：ΔV（瞬間速度變化量）、cycles/degree（每視角度可分辨線數）、Mauthner 細胞（逃命總開關神經元）。
- **§十四 整段重寫**：先白話點出本節在解決「同點打太密」與「中魚/跑魚化學警報」兩事，再給數字；ART／Q₁₀／警報素（Schreckstoff）／CDOM／死區／系統性閉口／接近率／C&R 全部就近講清楚，數值全保留。
- **build.py 術語表新增 5 個彈窗**：糖皮質激素受體 GR、習慣化／脫習慣化、口觸（Oral-test）、Shore 硬度、Lure-shyness；並把中文「警報素」掛到既有 Schreckstoff 條目。`脫習慣化` pattern 排在 `習慣化` 前避免拆字，已驗證整詞包覆且 inject 不二次注入術語表。
- 口觸用詞有出處（3A §4「口觸前測試 Oral-test」），非自創。未動 H3，TOC 維持 30。

---

## senior §十二 皮質醇基線刪無出處值＋補單位（2026-06-23）

**性質**：數值勘誤（刪無出處值，扶正為上游有出處值）＋補單位。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- **§十二 新魚皮質醇基線「1.68 ng/mL」無出處刪除**：該值在 3A 報告與白皮書全文皆查無出處，且與上游矛盾（3A §5 明列新魚放流 <30 天靜息基線 = 10–15 ng/mL）。刪除 1.68，新魚基線扶正為上游有出處的 **10–15 ng/mL**。原括號內的「放流期 10–15」即正確值，現升為主值。
- **補單位**：老魚格括號補 ng/mL，並將「實測 35–45」改寫為「台灣高壓計費池實測集中 35–45 ng/mL」，明確其為 20–40 ng/mL 廣義基線中的偏高子範圍（出處 VSUP-A06）。
- 兩列結構恢復平行（廣義基線＋情境子範圍）。未動 H3，TOC 維持 30。

---

## senior §九／§二十 CFF 兩段對齊去重（2026-06-23）

**性質**：去重＋互相指路（數值與兩段論點不變）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- CFF 原本在 §九（速度決策面向）與 §二十（色彩/寫實度面向）各自獨立定義一次。兩處數值本就一致、無矛盾，但定義重複且彼此不指路。
- **§九 CFF 段**加錨點 `senior-cff`，段尾補可點跳轉指向 §二十（色彩/寫實度面向）。
- **§二十 開頭**去掉重複的 CFF 定義，改為承接 §九＋可點跳回 `senior-cff`，並標明本段是 CFF 的另一面向（餌的色彩與寫實度）。
- 結果：CFF 定義只留一份（§九），§九 管速度、§二十 管配色，雙向頁內錨點可點互跳（沿用 §二 溶氧那組同樣機制）。未動 H3，TOC 維持 30。

---

## senior §九 補 CFF（魚眼更新率）但書（2026-06-23）

**性質**：內容補缺＋可讀性補白（沿用既有數值）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- **補第三部白話框的空頭支票**：第三部開頭白話框列了 CFF 為關鍵詞，但 §八–§十一 正文從未使用，解釋被遠放到第六部 §二十。於 §九 末尾補 CFF 但書，讓白話框在本部兌現。
- **補 §九 速度邏輯的承重缺口**：原 §九 只用「水色決定看久看清」一個軸講速度門檻，未提速度門檻有上限＝魚眼更新率，且 CFF 隨溫度/光線升降（高溫 62–68 Hz／低溫黃昏 35–42 Hz）。補雙端陷阱——低 CFF（冷/暗）快餌糊成拖影、追不到 → 冬天黃昏要放慢；高 CFF（暖/亮/清水）餌被看得太清楚像慢動作 → 慢通路識破 → 只跟不咬 → 靠抖停加速逼反射。修正原文把冷水速度門檻當四季通用的誤導。
- **加防誤導框**：明確「慢動作 ≠ 不動」，高 CFF 放掉假餌是因「被識破」非「看成靜止」，並以 free fall／dead-stick 照樣中魚反證「不動=不能吃=棄攻」這個常見錯誤說法（呼應 §十一）。
- 補無儀器判斷法（水溫＋天色推 CFF 高低）。沿用 §二十 既有 CFF 數值，未動 H3，TOC 維持 30。

---

## senior §十一 補軟餌對應與閘門段重拆（2026-06-22）

**性質**：可讀性補白＋戰術補全（數值不變）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- **§十一 攻擊軸向表「對應操作」補軟餌**：原表只給硬餌，補上各軸向的 free fall／同層軟餌對應——Up-Strike 補雷蛙、無鉛軟 jerkbait（fluke）慢沉；Forward-Strike 補 paddle tail（T 尾）同層平拖、軟 jerkbait 水平抽；Down-Strike 維持貼底 0–15 cm 慢拖並列出 Jig＋蝦／蟹軟蟲、Texas、Neko、Drop Shot（用白皮書既有詞「蝦／蟹軟蟲」，不自創翻譯）。
- **§十一 補 free fall 自成一類觸發**：輕量/無鉛軟餌慢沉＝無助力獵物，多在下沉段被咬；從魚視角是由上而下落入 → 走 Up／Forward 軸；觸底未中再轉 Down-Strike 拖。
- **§十一 仰攻閘門段重拆**：原 run-on 句拆為三道各自獨立的閘門＋冬天一道額外限制——溫度閘門（<12°C／南 <14°C 抑制、<10°C 停）、溶氧閘門（DO 1.6–2.0 mg/L 浮頭求生不索餌、100% 拒仰攻）、水色閘門（決定能從多深發動：北迎風 <30／南 <50／深水庫 <120 cm），外加冬天 <12°C 全面遲鈍（解析 −30–50%、掃描頻率 −65–70%、搜索錐 ±10°、橫向位移須 <10 cm）。數值全數保留。

---

## senior §十/§十一 感官接力與重疊錐邏輯修正（2026-06-22）

**性質**：邏輯修正＋可讀性補白（數值不變）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- **§十 開頭 NTU 句拆清楚**：能見度約 15 cm 是視覺退場線；不同水質到不同 NTU 才壓到 15 cm（褐 35／綠 45／洪水灰 60，概數 40），「<5 cm」是「到 15 cm 線時高解析視距早已剩 <5 cm」而非另一事件。補無儀器判斷法（餌沉約一掌深 ~15 cm 看不到即過線）。
- **§十「雙系統」改為三感官接力**：原文寫雙系統卻列三列（眼／側線／聽石），改為「由遠到近接力」——聽石遠場找區域 → 眼睛中場辨識 → 側線近場 <40 cm 觸發；明確涵蓋「餓又看不到時靠聽石找區域」。
- **§十一 重疊錐邏輯修正**：講明 25°–30° 是視野張角（非餌眼夾角、非弧度）；錐固定於魚頭前且偏上，俯攻是魚轉體把底餌帶進錐（解原「下方盲區」與俯攻軸向自相矛盾）；「只跟不咬」補後路——突發加速可走反射通路逼咬。
- **§十一 第三部精要改為四條**：速度通路／視覺退場線／三感官接力／光場軸向，光場條補「先讓餌進重疊錐、否則跟隨、可用變速走反射」。

---

## senior §九 反射咬段補通路定義與原因（2026-06-22）

**性質**：可讀性補白（數值不變）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- §九 反射咬段補三處原本只給結論未給解釋的缺口：(1) 明確定義快通路（視網膜→視頂蓋→網狀脊髓、不經前腦的反射弧）與慢通路（繞前腦/端腦做辨識與皮質醇避忌審查、上百毫秒）；(2) 解釋 >150 ms／≥1.2 m/s 速度臨界的「搶時間」邏輯與逃跑加速度 ≥2.5 m/s² 為何是快通路最強訊號；(3) 解釋水色速度差異原因（清水看久看清→慢通路強→靠抖停加速突發訊號；濁水看晚看糊→靠絕對高速被看到，越濁越快 1.5/1.2/1.1）。數值全數保留。未動 H3。

---

## senior §八 OFT 段重新梳理（2026-06-22）

**性質**：結構梳理（數值不變）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- §八 OFT 段重排，對齊標題的兩個提問：開頭先補 OFT 核心原則（挑「最划算」非「最大」＝CP 值＝能量 ÷ 處理時間），再以粗體分段為「**為什麼挑食**」（尺寸篩選／軟殼期暴利兩條）與「**什麼時候不挑**」（門檻隨水溫三條對照：25–27°C 最不挑／>30°C 變挑／<18°C 守株待兔）。修掉原「兩個衍生規則」涵蓋不到後段的問題；冬季反轉偏好接於低溫段後維持連貫。未動 H3，TOC 數不變。

---

## senior 挑食門檻段補因果鏈（2026-06-22）

**性質**：可讀性補白（數值不變）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- 「挑食門檻隨水溫移動」段把「高溫每次追擊機會成本極高」這句濃縮敘述攤開成因果鏈：點明活性高峰在最適溫 25–27°C，>30°C 是過熱（怠速代謝高＋高溫缺氧＋衝刺後還氧債慢＝失敗一次太貴），魚才改打效率仗、需精準擬真；並補低溫 <18°C 反向邏輯（巡游搜尋累→守株待兔→不挑食）。正面敘述，不用破除迷思句式。

---

## senior 反射咬加速度句補白話（2026-06-18）

**性質**：微調可讀性（數值不變）。**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

- 「逃跑加速度 ≥2.5 m/s²」的括號由「150–200 ms 內從 0.5 加到 1.0 m/s 以上」改為「**也就是說，速度從 0.5 m/s、在 150–200 ms 內加速到 1.0 m/s 以上**」，把抽象加速度講成白話的速度變化。

---

## §七 風生流重寫＋風的現場判讀＋術語統一（2026-06-17）

**性質**：可讀性重寫＋術語統一＋新增卡（數值保留為背景）。
**目標檔案**：`台灣大嘴黑鱸白皮書-{junior,senior,expert}.md`、`build.py`、`docs/index.html`

| 項目 | 說明 |
|------|------|
| senior §七 整節重寫 | 食物聚在下風岸（看泡沫線／被風浪打的岸）＝找魚重點；分水體（淺塘從表到底動；深水庫夏季只在躍溫層以上、秋冬混合整層；一律打下風岸）；風強弱**看水面判斷**（鏡面無風／細紋小浪＝甜區／浪大拍岸、控不住餌＝過頭、魚退避風緩流死角）。流速 5–12／>20 cm/s 降為背景；去除自我懷疑與縮字 |
| 術語統一「下風岸」 | 全書統一上風／下風（**下風岸＝食物堆積那岸**）：senior「迎風岸」（標題／精要／推理）全改下風岸、junior 同句去重、GLOSSARY 風生流／rheotaxis 卡同步。氣候區「北部迎風（面）」不動 |
| 不用「水柱」 | 「全水柱／倒進水柱」改「整片水」：senior 3 處、expert L86 |
| 新增風應力卡 | GLOSSARY 加 `風應力` 卡並自動連結 |
| junior／expert | junior 加泡沫線白話；expert 第四部加「風生流×水體×季節」校準 |
