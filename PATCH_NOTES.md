...
## senior 棲位表補管理池註解（2026-06-16）

**性質**：補完整性（管理池說明，不捏造季節深度）。
**目標檔案**：`台灣大嘴黑鱸白皮書-senior.md`、`docs/index.html`

| 項目 | 說明 |
|------|------|
| 管理池註解 | 棲位（季節深度）表原僅 4 列（野塘×2、水庫×2），於表下加註解交代六大水體裡的管理池：其棲位由**水車開關＋毒區**決定、非季節，故不另填四欄——水車運轉貼結構／出水口可貼底、停機上浮 0–30 cm、南部夏季 H₂S 頂到離底 ≥100 cm。junior（白話線索）／expert（劇本一/二涵蓋管理池）未動 |

---

## 去「自我修正」語氣＋棲位基準/躍溫層補正（2026-06-16）

**性質**：可讀性改寫（改成正面敘述，無數值變動）。
**目標檔案**：`台灣大嘴黑鱸白皮書-{junior,senior,expert}.md`、`docs/index.html`

| 項目 | 說明 |
|------|------|
| 去自我質疑語氣 | 把幾處「自問自答／反駁自己」框架改成正面敘述：junior「破除迷思…？不會」→「連續風雨魚會怎樣？」；senior「三個常被誤推的點」→「三個要分清楚的關鍵」、兩處「別量／別管 lux」→「用看得到的判斷」；expert「斷崖線不是固定鐘點」→「斷崖線位置會移動」；senior「（修正北部可隨意貼底舊觀念）」整段改寫成正面季節事實 |
| 避毒高度講清楚 | senior「南浮北底」段重寫：點明「離底 X cm＝底下 X 公分是危險帶、停在以上到水面都行（無上限）」；南部只留 H₂S ~100 cm（Fe²⁺ 包在內、不另列），北部 25–67 cm 解釋成危險帶高度（非上限） |
| 棲位基準標註（Q1） | 棲位表摘要行標明「數字＝水深、從水面往下算；避毒『離底 X cm』從水底往上算」 |
| 南部水庫躍溫層（Q2） | 棲位表南部水庫夏季補「躍溫層上緣」 |
| junior/expert | 餘處基準已明確、未動 |
